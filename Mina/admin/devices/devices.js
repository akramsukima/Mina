    ...
    for (var key of Object.keys(Devices)) {
        ...
        if (match) {
            ...
            <div id="device">
                ...
                <h6>SerialNo. ${Devices[key].Seriannummer}</h6>
                <h6>IMEINo. ${Devices[key].IMEI}</h6>
                <h6>${Devices[key].Storage}gb  ${Devices[key].Batterie}%</h6>
                <button onclick=details('${key}')>Details</button>
            </div>
            `
    ...
    for (var key of Object.keys(doc.data())) {
        if (key == 'CurrentID') {
            ...
            <div id="device">
                ...
                alt="https://assets.swappie.com/cdn-cgi/image/width=600,height=600,dpr=2,fit=contain,format=auto/swappie-iphone-xr-black.png?v=11">
                <h3>${Device.Model}</h3>
                <h6>SerialNo. ${Device.Seriannummer}</h6>
                <h6>IMEINo. ${Device.IMEI}</h6>
                <h6>${Device.Storage}gb  ${Device.Batterie}%</h6>
                <button onclick=details('${key}')>Details</button>
            </div>
            `
